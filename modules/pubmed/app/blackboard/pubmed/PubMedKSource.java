...
public class PubMedKSource implements KSource, KType {
            ...
            for (int j = 0; j < nodes.getLength(); j++) {
                ...
                if (headings.getLength() > 0) {
                    ...
                    String[] treeNums = cache.getOrElse
                        ("treeNumbers/"+meshId, new Callable<String[]> () {
    ...
    public KNode resolvePubmed (String pmid, KNode kn, KGraph kg)
        throws Exception {
        KNode dn = createPubMedNodeIfAbsent (pmid, kg);
        if (dn != null)
    ...
    public MeSH[] searchMeSH (String query) throws Exception {
        ...
        WSRequest req = wsclient.url("https://id.nlm.nih.gov/mesh/sparql")
             ...
             */
             ...
             " ?d meshv:active 1 .\n"+
             " ?d rdfs:label ?dName .\n"+
             " ?d rdfs:label \""+query.replaceAll("%20"," ")+"\"@en"+
             " }\n"+
             " ORDER BY ?d ?treeNumber\n")
            .setQueryParameter("format", "json")
            .setQueryParameter("limit", "50")
            .setQueryParameter("year", "current")
